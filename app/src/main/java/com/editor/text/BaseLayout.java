...
{
	private static final int LineLeftPadding = 10;
	private static final int LineRightPadding = 50;
	...
	public void draw(Canvas canvas, int cursorOffsetVertical)
	{
		...
		int lineHeight = (int)((font.bottom-font.top) * mLineSpacing);
		int tyOffset = (int)(((font.bottom-font.top) * (mLineSpacing - 1) / 2) - font.ascent);

		//在0的左侧绘制行数，绘制的是行号而不是行的下标
		if(see.left < 0){
			float leftPadding = getLineNumberWidth();
			int highlightLine = cursorOffsetVertical / lineHeight;
			drawLineNumber(startLine+1, endLine+1, highlightLine+1, -leftPadding+LineLeftPadding, startLine*lineHeight, lineHeight, tyOffset, canvas, mPaint);
		}
	...
	{
		for(; start < end; ++start){
			if(canDrawAt(chars, table, widths, start)){
				break;
			}
		}
		return start;
	...
	private static boolean canDrawAt(char[] chars, char[] table, float[] widths, int i){
		return table[i] == FREE && chars[i] != FT && widths[i] > 0;
	}
	...
	public final float getLineNumberWidth(){
		return mPaint.measureText(String.valueOf(getLineCount())) + LineLeftPadding + LineRightPadding;
	}
	...
	{
		float cursorWidthSpacing = 0.1f;
		float width = cursorWidthSpacing * mPaint.getTextSize();
		int lineHeight = getLineHeight();
